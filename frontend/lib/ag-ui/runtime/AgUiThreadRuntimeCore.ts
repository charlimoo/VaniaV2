...
export class AgUiThreadRuntimeCore {
  ...
  updateOptions(options: Omit<CoreOptions, "notifyUpdate">) {
    if (
      (options.threadId && options.threadId !== this.threadId) ||
      (options.agentId && options.agentId !== this.agentId)
    ) {
      console.log(`[Core] Switching context: Agent=${options.agentId}, Thread=${options.threadId}`);
      this.threadId = options.threadId ?? "main";
      this.agentId = options.agentId;
      this.messages = [];
  ...
  applyExternalMessages(messages: readonly ThreadMessage[]): void {
    console.log(`[Core] Applying ${messages.length} messages.`);
  ...
  private async startRun(
    ...
    // --- MODIFIED ABORT LISTENER (ACTIVE CANCELLATION) ---
    abortSignal.addEventListener(
      ...
      () => {
        ...
        // [NEW] Call Backend Cancel Endpoint to ensure server-side kill
        // Only trigger if we have a valid runId and agentId
        if (this.agentId && runId) {
            const token = localStorage.getItem("accessToken");
            // Determine API URL (Use environment var or fallback to default)
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
            
            if (token) {
                fetch(`${apiUrl}/agent/runs/${runId}/cancel`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                }).catch(err => console.error("[Core] Failed to notify backend cancel:", err));
            }
        }
      },
      { once: true },
