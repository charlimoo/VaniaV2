...
const ReasoningGroupImpl: ReasoningGroupComponent = ({
  ...
  const isReasoningStreaming = useAssistantState(({ message }) => {
    ...
    // Check if the current reasoning group is the one actively streaming
    return lastType === "reasoning" && lastIndex >= startIndex && lastIndex <= endIndex;
  });

  // [FIX 3] Check if there is actual content within the reasoning part
  // In v0.11, children will be empty/null during the initial reasoning delay
  const hasContent = !!children;

  return (
    <ReasoningRoot>
      {/* Show "Analyzing..." only when streaming and content hasn't arrived yet */}
      {!hasContent && isReasoningStreaming ? (
        <div className="flex items-center gap-2 py-2 text-sm text-muted-foreground animate-pulse mb-2" dir="rtl">
