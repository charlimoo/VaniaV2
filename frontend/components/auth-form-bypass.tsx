// start of components/auth-form-bypass.tsx
"use client"
...
export function AuthForm({ className, onSuccess, ...props }: AuthFormProps) {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resVerify = await fetch(`${API_BASE_URL}/api/auth/verify-otp/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (data.user_created) {
        ...
        if (!result.success) throw new Error("بارگذاری پروفایل انجام نشد")

        setStep("ONBOARDING")
      } else {
        // Clear tokens to force password entry
    ...
    <form onSubmit={handleRequestOtp} className="flex flex-col gap-6">
      ...
      <div className="grid gap-4">
        <div className="grid gap-2">
          ...
          <Input
            id="phone"
            name="username"           // ADDED: Helps browser identify user early
    ...
    <form onSubmit={handleVerifyOtp} className="flex flex-col gap-6">
      ...
      <div className="grid gap-4">
        <div className="grid gap-2">
          ...
          <Input
            id="otp"
            name="one-time-code"      // ADDED: Standard OTP tag
            autoComplete="one-time-code" // ADDED
            type="text"
            placeholder="123456"
            value={otpCode}
  ...
  const renderPasswordStep = () => (
    <form onSubmit={handlePasswordLogin} className="flex flex-col gap-6">
      ...
      <div className="grid gap-4">
        ...
        >
          <ArrowRight className="h-4 w-4" /> بازگشت به شروع
        </Button>
      </div>
    </form>
  )
  ...
  const renderFormContent = () => (
    <div className="flex flex-col justify-center p-6 md:p-12 bg-background relative overflow-hidden h-full">
      ...
      <div className="relative z-10 max-w-[380px] mx-auto w-full">
        ...
        {step === "ONBOARDING" && (
          // CHANGED: Pass phoneNumber to OnboardingForm
          <OnboardingForm 
...
// end of components/auth-form-bypass.tsx